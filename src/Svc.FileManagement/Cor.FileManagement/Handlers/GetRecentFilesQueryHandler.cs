// E:\untitled46\RST_ERP\src\Svc.FileManagement\Cor.FileManagement\Handlers\GetRecentFilesQueryHandler.cs
...
public class GetRecentFilesQueryHandler : IRequestHandler<GetRecentFilesQuery, List<FileDocumentDto>>
{
    private readonly FileDbContext _context;

    public GetRecentFilesQueryHandler(FileDbContext context)
    {
        _context = context;
    }

    public async Task<List<FileDocumentDto>> Handle(GetRecentFilesQuery request, CancellationToken cancellationToken)
    {
        var recentFiles = await _context.FileAccessLogs
            .Where(l => l.UserId == request.UserId && l.Action == "View" && l.DocumentId.HasValue)
        ...
        var documents = await _context.FileDocuments
            .Where(f => recentFiles.Contains(f.Id) && !f.IsDeleted)
            .OrderByDescending(f => f.DateMod)
            .Take(request.Count)
            .ToListAsync(cancellationToken);

...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IAuthMngrRepo<TEntity> Repository<TEntity>() where TEntity : BaseEntity
    {
        return (IAuthMngrRepo<TEntity>)_repositories.GetOrAdd(typeof(TEntity), _ =>
        {
            _logger.LogInformation("Creating new AuthMngrRepo<{EntityType}> instance for UnitOfWork.", typeof(TEntity).Name);
            var repoLogger = _loggerFactory.CreateLogger<AuthMngrRepo<TEntity>>();
            return new AuthMngrRepo<TEntity>(_context, repoLogger);
        });
    }

...
public class RabbitMQEventPublisher : IEventPublisher, IDisposable
{
    ...
    public RabbitMQEventPublisher(IConfiguration configuration, ILogger<RabbitMQEventPublisher> logger)
    {
        ...
        // Declare exchanges
        _channel.ExchangeDeclare("crm.events", ExchangeType.Topic, durable: true);
    ...
    public async Task PublishAsync<T>(string entityName, string eventType, T data, CancellationToken ct = default)
    {
        try
        {
            ...
            var routingKey = $"crm.{entityName.ToLower()}.{eventType.ToLower()}";

            _logger.LogInformation("📤 Publishing to exchange: crm.events, routingKey: {RoutingKey}", routingKey);

...
public sealed class DapperHelper : IDapperHelper
{
    ...
    public async Task<IEnumerable<TResult>> QueryAsync<TFirst, TSecond, TThird, TResult>(string sql, Func<TFirst, TSecond, TThird, TResult> map, object? param = null, string splitOn = "Id", CancellationToken ct = default, bool buffered = true)
    {
        var sw = Stopwatch.StartNew();
        return await _retry.ExecuteAsync(async () =>
        {

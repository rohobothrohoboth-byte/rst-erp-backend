...
public class UnitOfWork : IUnitOfWork
{
    ...
    public ICoreModuleRepo<TEntity> Repository<TEntity>() where TEntity : BaseEntity
    {
        return (ICoreModuleRepo<TEntity>)_repositories.GetOrAdd(typeof(TEntity), type =>
        {
            _logger.LogInformation("Creating new CoreRepository<{EntityType}> instance for UnitOfWork.", typeof(TEntity).Name);
            var repoLogger = _loggerFactory.CreateLogger<CoreModuleRepo<TEntity>>();
            return new CoreModuleRepo<TEntity>(_context, repoLogger);
        });
    }

...
public class GetCompsHandler : IRequestHandler<AllCompsQry, List<CompListDto>>
{
    ...
    public async Task<List<CompListDto>> Handle(AllCompsQry request, CancellationToken ct)
    {
        ...
        var qb = new QueryBuilder()
            .Select<Company>(c, x => x.Id, x => x.Name, x => x.NameAm, x => x.TaxId!, x => x.Phone!, x => x.Email!, x => x.Address!, x => x.Website!, x => x.LogoUrl!, x => x.Motto!, x => x.Mission!, x => x.Vision!, x => x.Values!, x => x.Structure!, x => x.DateAdd, x => x.DateMod!, x => x.xmin)

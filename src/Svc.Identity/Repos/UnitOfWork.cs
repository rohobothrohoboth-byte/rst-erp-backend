...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IAuthRepo<TEntity> Repository<TEntity>() where TEntity : BaseEntity
    {
        return (IAuthRepo<TEntity>)_repositories.GetOrAdd(typeof(TEntity), _ =>
        {
            _logger.LogInformation("Creating new CorHRMMRepo<{EntityType}> instance for UnitOfWork.", typeof(TEntity).Name);
            var repoLogger = _loggerFactory.CreateLogger<AuthRepo<TEntity>>();
            return new AuthRepo<TEntity>(_context, repoLogger);
        });
    }

...
public class AuditLogConsumerService : BackgroundService
{
    ...
    private async Task ProcessAuditLogAsync(AuditLogEventDto auditEvent)
    {
        try
        {
            ...
            var auditLog = new AuditLog
            {
                ...
                OldValues = EnsureValidJson(auditEvent.OldValues),
                NewValues = EnsureValidJson(auditEvent.NewValues),
                ChangesJson = EnsureValidJson(auditEvent.ChangesJson),
                MetadataJson = EnsureValidJson(auditEvent.MetadataJson),
                RequestHeaders = EnsureValidJson(auditEvent.RequestHeaders),
                ...
                CreatedByUserId = auditEvent.CreatedByUserId,
                CreatedByUserName = auditEvent.CreatedByUserName,
                DateMod = DateTime.UtcNow,
                PeriodId = auditEvent.PeriodId,
                QueryString = auditEvent.QueryString,
                RowVersion = Guid.NewGuid().ToString(),
                UpdatedByUserId = auditEvent.UpdatedByUserId,
                UpdatedByUserName = auditEvent.UpdatedByUserName,
    ...
    /// <summary>
    /// Ensures that a string is valid JSON. Returns "{}" for null, empty, or invalid JSON.
    /// </summary>

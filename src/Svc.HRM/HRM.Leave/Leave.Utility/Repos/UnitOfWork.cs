...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IHrmLeaveRepo<TEntity> Repository<TEntity>() where TEntity : BaseEntity
    {
        return (IHrmLeaveRepo<TEntity>)_repositories.GetOrAdd(typeof(TEntity), type =>
        {
            _logger.LogInformation("Creating new HrmLeaveRepo<{EntityType}> instance for UnitOfWork.", typeof(TEntity).Name);
            var repoLogger = _loggerFactory.CreateLogger<HrmLeaveRepo<TEntity>>();
            return new HrmLeaveRepo<TEntity>(_context, repoLogger);
        });
    }

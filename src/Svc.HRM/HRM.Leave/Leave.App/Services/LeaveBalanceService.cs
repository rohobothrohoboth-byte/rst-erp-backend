...
{
    Task<double> GetBalance(Guid employeeId, Guid leaveTypeId);
    //Task EnsureSufficient(Guid employeeId, Guid leaveTypeId, double days);
}
...
public sealed class LeaveBalanceService : ILeaveBalanceService
{
    ...
    //public async Task EnsureSufficient(Guid employeeId, Guid leaveTypeId, double days)
    //{
    //    var balance = await GetBalance(employeeId, leaveTypeId);

...
public class EmpPhotoByIdQryHandler : IRequestHandler<EmpPhotoByIdQry, EmpPhotoDto?>
{
    ...
    public async Task<EmpPhotoDto?> Handle(EmpPhotoByIdQry request, CancellationToken cancellationToken)
    {
        ...
        var c = new EmpPhotoDto
        {
            ...
            EmpFullName = emp != null ? $"{emp.Person.FirstName} {emp.Person.MiddleName} {emp.Person.LastName}" : "NOT AVAILABLE",
            EmpFullNameAm = emp != null ? $"{emp.Person.FirstNameAm} {emp.Person.MiddleNameAm} {emp.Person.LastNameAm}" : "መረጃ ማግኘት አልተቻለም",

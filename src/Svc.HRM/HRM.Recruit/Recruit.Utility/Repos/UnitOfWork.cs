...
public class UnitOfWork : IUnitOfWork
{
    ...
    public IHrmRecruitRepo<TEntity> Repository<TEntity>() where TEntity : BaseEntity
    {
        return (IHrmRecruitRepo<TEntity>)_repositories.GetOrAdd(typeof(TEntity), type =>
        {
            _logger.LogInformation("Creating new HrmRecruitRepo<{EntityType}> instance for UnitOfWork.", typeof(TEntity).Name);
            var repoLogger = _loggerFactory.CreateLogger<HrmRecruitRepo<TEntity>>();
            return new HrmRecruitRepo<TEntity>(_context, repoLogger);
        });
    }

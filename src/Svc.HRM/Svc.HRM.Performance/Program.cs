...
using Serilog;
using Svc.HRM.Performance.Extensions;
...
var portString = GetConfig("ServiceUrls:PerformanceApi", "https://localhost:7016");
...
var dbConnectionString = GetConfig("ConnectionStrings:HrmPerformanceDb", null);
if (string.IsNullOrEmpty(dbConnectionString))
{
    ...
    dbConnectionString =
        $"Host={dbHost};Port={dbPort};Database=HRM.PerformanceDb;Username={dbUser};Password={dbPassword};Include Error Detail=true";
}
...
builder.Services.AddDbContext<PerformanceDbContext>(options => options.UseNpgsql(dbConnectionString));
...
builder.Services.AddHealthChecks().AddDbContextCheck<PerformanceDbContext>();
...
app.UseMiddleware<Svc.HRM.Performance.Middleware.ApiExceptionMiddleware>();
app.UseAuthentication();
app.UseAuthorization();
app.MapControllers();
app.MapHealthChecks("/health");

Console.WriteLine($"Performance Service starting on https://0.0.0.0:{port}");
await app.RunAsync();

...
public class RabbitMQEventPublisher : IEventPublisher, IDisposable
{
    ...
    public RabbitMQEventPublisher(IConfiguration configuration, ILogger<RabbitMQEventPublisher> logger)
    {
        ...
        _channel.ExchangeDeclare("finance.events", ExchangeType.Topic, durable: true);
    }
    ...
    public async Task PublishPayrollProcessedAsync(Guid payrollRunId, string payrollName, CancellationToken ct = default)
    {
        try
        {
            ...
            var routingKey = $"finance.payroll.processed";

            _logger.LogInformation("📤 Publishing payroll processed event: PayrollRunId: {PayrollRunId}, PayrollName: {PayrollName}",
                payrollRunId, payrollName);
            ...
            _logger.LogInformation("✅ Published payroll processed event for: {PayrollName} with CorrelationId: {CorrelationId}",
                payrollName, eventData.CorrelationId);

            await Task.CompletedTask;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to publish payroll processed event: {PayrollRunId} {PayrollName}",
                payrollRunId, payrollName);
            throw;
        }
    }
